...
def parse_video_description(raw_text):
    ...
    Expected format from prompt:
        ...
        ANGLE: <camera angle>
        SKILL LEVEL: <level>
        PHASES VISIBLE: <comma-separated phases>
    ...
    # ── Normalize verbose Pegasus output to short keywords ──
...
def _normalize_technique(val):
    ...
    for k in known:
        if k in val_lower:
            return k
    # If no known match, take the first few words
...
def _find_near_duplicates_via_search(
    ...
    Since Marengo search returns rank (ordinal) rather than a similarity
    score, we flag pairs where one video's description retrieves the
    other video within the top `threshold_rank` results.

    Returns list of dicts: {video_a, video_b, note}.
    """
    near_dupes = []
    seen_pairs = set()
        ...
        try:
            ...
            for item in results:
                if item.video_id == source_video_id:
                    continue
                # Find which sample this matched video belongs to
                    ...
                    if (
                        ...
                        if pair_key not in seen_pairs:
                            ...
                            near_dupes.append({
                                ...
                                "video_b": descriptions[j].get(
                                    "filepath", f"video_{j}"
                                ),
                                "note": (
                                    f"Matched within top {threshold_rank} "
                                    f"search results — likely similar content"
                                ),
                            })
                        break
                # Only check top threshold_rank results
                if item.rank and item.rank > threshold_rank:
                    break
        except Exception:
            # Search may fail if index isn't ready; continue profiling
            continue
...
def _find_near_duplicates_text(descriptions, threshold=0.6):
    """Fallback near-duplicate detection using text similarity of
    Pegasus descriptions (Jaccard on word sets).

    Returns list of dicts: {video_a, video_b, similarity}.
    """
    near_dupes = []
...
def profile_references(dataset, sport, client, index_id=None, similarity_threshold=0.6):
    """Generate a DNA health card for a reference video dataset.

    Args:
        dataset: FiftyOne dataset of reference videos. Each sample should
            have a 'tl_video_id' field (indexed in Twelve Labs).
        sport: Sport name (e.g. "boxing", "tennis").
        client: Initialized TwelveLabs client.
        index_id: Twelve Labs index ID for search-based duplicate
            detection.  If None, falls back to text-based similarity.
        similarity_threshold: Jaccard threshold for text-based
            near-duplicate detection (0-1, default 0.6).
